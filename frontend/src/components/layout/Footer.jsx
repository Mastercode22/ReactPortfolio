...
import { ArrowUp, Github, Linkedin, Twitter, Mail, Send, Check, Globe } from 'lucide-react';
...
export const Footer = () => {
  ...
  useEffect(() => {
    getSettings().then(res => res && setSettings(res)).catch(err => console.error(err));
    ...
    <footer className="relative pt-20 pb-12 border-t border-white/10 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 pb-16 border-b border-white/10">
          ...
          <div className="md:col-span-5 space-y-4">
            ...
            >
              ...
              />
              <span className="font-extrabold text-xl tracking-tight text-[#1B2430] dark:text-[#F8FAFC] ml-2">
                {settings?.site_name || "Emmanuel Quarshie"}
              </span>
            </motion.div>

            <p className="text-sm text-[#667085] dark:text-[#CBD5E1] max-w-md leading-relaxed">
              {settings?.short_description || "Crafting world-class digital experiences blending Neumorphic depth, Glassmorphism, and Apple-grade precision engineering."}
            </p>
        ...
        <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-[#667085] dark:text-[#CBD5E1]">
          <p>
            {settings?.copyright_text || `© ${new Date().getFullYear()} Emmanuel Quarshie. All rights reserved.`}
          </p>
