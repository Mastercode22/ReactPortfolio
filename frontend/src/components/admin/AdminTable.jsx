...
const AdminTable = ({ columns, data, onEdit, onDelete, isLoading, emptyMessage = 'No data found' }) => {
          ...
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="flex gap-4 p-4 border-b border-white/5 last:border-0">
              {columns.map((_, colIndex) => (
                <div key={colIndex} className="h-6 bg-white/5 rounded-md flex-1"></div>
              ))}
  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-[#121620] border border-white/10 rounded-2xl p-12 text-center shadow-xl">
        <div className="text-4xl mb-4">📭</div>
        <h3 className="text-xl font-medium text-white mb-2">{emptyMessage}</h3>
        <p className="text-[#CBD5E1]">Click the add button above to get started.</p>
      </div>
    );
  }

  return (
    <div className="bg-[#121620] border border-white/10 rounded-2xl overflow-hidden shadow-xl overflow-x-auto">
          ...
          {data.map((row, rowIndex) => (
            <tr key={rowIndex} className="hover:bg-white/[0.02] transition-colors">
              {columns.map((col, colIndex) => (
                <td key={colIndex} className="px-6 py-4 text-[#F8FAFC]">
